...
extension NibLoadable {
    ...
    func fromNib() -> UIView? {
        ...
        self.addSubview(contentView ?? UIView())
    ...
    func fromNib(_ xibName: String) -> UIView? {
        let contentView = Bundle(for: type(of: self)).loadNibNamed(xibName, owner: self, options: nil)?.first as? UIView
        self.addSubview(contentView ?? UIView())
        contentView?.translatesAutoresizingMaskIntoConstraints = false
        contentView?.edges(to: self)
        return contentView
...
extension UITableView {
    ...
    func dequeueReusableCell<T: UITableViewCell>(forIndexPath indexPath: IndexPath) -> T where T: ReusableView {
        guard let cell = dequeueReusableCell(withIdentifier: T.reuseIdentifirer, for: indexPath) as? T else {
